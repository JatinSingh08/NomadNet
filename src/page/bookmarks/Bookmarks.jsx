...
const Bookmarks = () => {
  ...
  const userBookmarkPosts = usersData?.find(user => user.username === foundUser.username)?.bookmarks;
  const bookmarkPosts = postsData?.filter(({_id}) => userBookmarkPosts?.some(post => post._id === _id));
      ...
      <div className='flex flex-col gap-8'>
        {
          ...
          : (
            <h1 className='text-2xl font-medium text-slate-400 m-auto mt-10'>No Posts Bookmarked</h1>
          )
        }
      </div>
    </div>
