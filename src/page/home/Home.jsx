...
const Home = () => {
  ...
  const userFollowingPosts = postsData?.filter(
    (post) =>
      post?.userId === user?._id ||
      user?.following?.some((userData) => userData._id === post.userId)
  );
  ...
  return (
    <div className="px-20 py-10 flex flex-col gap-8  w-[calc(100%-46rem)] min-h-screen  ml-[22rem]">
      ...
      <div className="w-full card rounded-lg h-14 flex justify-between">
        ...
        <button
          className={`flex items-center justify-center gap-2 text-lg w-[50%] hover:bg-rose-100 hover:rounded-s-2xl ${
            postsOrder === "latest" && "bg-rose-100 rounded-l-2xl"
          }`}
